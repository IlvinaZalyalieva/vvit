...
class MainWindow(QWidget):
    ...
    def _connect_to_db(self):
        self.conn = psycopg2.connect(database="sc8",
                                         user="postgres",
    ...
    def _update_week_table(self):
        self.cursor.execute("SELECT * FROM firsttt_week ORDER BY id")
    ...
    def _update_teacher_table(self):
        self.cursor.execute("SELECT * FROM teacherr ORDER BY id")
        records = list(self.cursor.fetchall())
    ...
    def _update_subject_table(self):
        self.cursor.execute("SELECT * FROM subjectss ORDER BY id")
        records = list(self.cursor.fetchall())
    ...
    def _update_schedule_table(self):
        self.cursor.execute('SELECT firsttt_week.day, firsttt_week.timee, firsttt_week.subjectss, '
                            'teacherr.name, firsttt_week.room FROM firsttt_week JOIN teacherr ON firsttt_week.subjectss = teacherr.subjectss WHERE firsttt_week.day = \'пн\' ORDER BY firsttt_week.id;')
        records = list(self.cursor.fetchall())
    ...
    def _update_schedule_table2(self):
        self.cursor.execute('SELECT firsttt_week.day, firsttt_week.timee, firsttt_week.subjectss, '
                            'teacherr.name, firsttt_week.room FROM firsttt_week JOIN teacherr ON firsttt_week.subjectss = teacherr.subjectss WHERE firsttt_week.day = \'вт\' ORDER BY firsttt_week.id;')
        records = list(self.cursor.fetchall())
    ...
    def _update_schedule_table3(self):
        self.cursor.execute('SELECT firsttt_week.day, firsttt_week.timee, firsttt_week.subjectss, '
                            'teacherr.name, firsttt_week.room FROM firsttt_week JOIN teacherr ON firsttt_week.subjectss = teacherr.subjectss WHERE firsttt_week.day = \'ср\' ORDER BY firsttt_week.id;')
        records = list(self.cursor.fetchall())
    ...
    def _update_schedule_table4(self):
        self.cursor.execute('SELECT firsttt_week.day, firsttt_week.timee, firsttt_week.subjectss, '
                            'teacherr.name, firsttt_week.room FROM firsttt_week JOIN teacherr ON firsttt_week.subjectss = teacherr.subjectss WHERE firsttt_week.day = \'чт\' ORDER BY firsttt_week.id;')
        records = list(self.cursor.fetchall())
    ...
    def _update_schedule_table5(self):
        self.cursor.execute('SELECT firsttt_week.day, firsttt_week.timee, firsttt_week.subjectss, '
                            'teacherr.name, firsttt_week.room FROM firsttt_week JOIN teacherr ON firsttt_week.subjectss = teacherr.subjectss WHERE firsttt_week.day = \'пт\' ORDER BY firsttt_week.id;')
        records = list(self.cursor.fetchall())
    ...
    def _update_schedule_table6(self):
        self.cursor.execute('SELECT firsttt_week.day, firsttt_week.timee, firsttt_week.subjectss, '
                            'teacherr.name, firsttt_week.room FROM firsttt_week JOIN teacherr ON firsttt_week.subjectss = teacherr.subjectss WHERE firsttt_week.day = \'сб\' ORDER BY firsttt_week.id;')
        records = list(self.cursor.fetchall())
    ...
    def _delete_day_from_week(self, rowNum, day):
        ...
        for i in range(self.week_table.columnCount()):
            try:
                ...
                row.append(None)
        try:
            self.cursor.execute(f'UPDATE firsttt_week SET room = \'\', subjectss = \'\' WHERE day = \'{row[0]}\' AND timee = \'{row[1]}\'')
            self.conn.commit()
    ...
    def _delete_day_from_teacher(self, rowNum, day):
        ...
        if row[2] in row2:
            ...
            self.cursor.execute(f'DELETE FROM teacherr WHERE id = \'{row[0]}\'')
            self.conn.commit()
            self._update_shedule2()
    ...
    def _delete_day_from_subject(self, rowNum, day):
        ...
        if row[1] in row3:
            ...
            self.cursor.execute(f'DELETE FROM subjectss WHERE id = \'{row[0]}\'')
            self.conn.commit()
            self._update_shedule3()
    ...
    def _change_day_from_week(self, rowNum, day):
        ...
        if not new_name in row2:
            ...
            self.cursor.execute(
                f'UPDATE firsttt_week SET room = \'{new_aud}\', subjectss = \'{new_name}\' WHERE day = \'{row[0]}\' AND timee = \'{row[1]}\'')
            self.conn.commit()
            self._update_shedule()
    ...
    def _change_day_from_teacher(self, rowNum, day):
        ...
        if r[2] in row2:
            ...
            self.cursor.execute(
                f'UPDATE teacherr SET name = \'{new_name}\' WHERE id = \'{r[0]}\'')
            self.conn.commit()
            self._update_shedule2()
        else:
            ...
            while True:
                if not new_subject in row3:
                    ...
                    if new_subject == r[2]:
                        self.cursor.execute(
                            f'UPDATE teacherr SET name = \'{new_name}\', subjectss = \'{new_subject}\' WHERE id = \'{r[0]}\'')
                        self.conn.commit()
                        self._update_shedule2()
                        break
                    else:
        ...
        if row2[1] in row3:
            ...
            while True:
                if new_subject in row:
                    ...
                    self.cursor.execute(f'UPDATE subjectss SET name = \'{new_subject}\' WHERE id = {row2[0]};')
                    self.conn.commit()
                    self._update_shedule3()
                    break
    ...
    def _add_shedule(self):
        ...
        if len(row2) == len(row):
            ...
            new_name, ok_pressed = QInputDialog.getText(self, "!", "Введите нового учителя:")
            new_subject, ok_pressed = QInputDialog.getText(self, "!", "Введите предмет этого учителя:")
            while True:
                if not new_subject in row2:
                    ...
                    self.cursor.execute(f'INSERT INTO teacherr (name, subjectss) VALUES (\'{new_name}\', \'{new_subject}\');')
                    self.conn.commit()
                    self._update_shedule2()
                    break
    ...
    def _add_shedule2(self):
        ...
        for i in range(40):
            try:
                ...
                row.append(None)
        new_name, ok_pressed = QInputDialog.getText(self, "Привет", "Введите новый предмет:")
        while True:
            if new_name in row:
                new_name, ok_pressed = QInputDialog.getText(self, "!", "Данный предмет уже существутет\nпопробуйте еще раз")
            else:
                self.cursor.execute(f'INSERT INTO subjectss (name) VALUES (\'{new_name}\');')
                self.conn.commit()
                self._update_shedule3()
                break
