...
class Calculator(QWidget):
    ...
    def _result(self):
        if (self.input.text().count('.') > 1) or (self.num_1.count('.') > 1) or \
                (any(c.isalpha() for c in self.input.text())) or (any(c.isalpha() for c in self.num_1)) \
                or (self.input.text().count(' ') > 0) or (self.num_1.count(' ') > 0):
